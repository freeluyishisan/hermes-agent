...
The picker reads the live catalog from [models.dev](https://models.dev) and merges in a curated fallback (`mistral-large-latest`, `mistral-medium-latest`, `mistral-small-latest`, `codestral-latest`, `devstral-medium-latest`, `ministral-8b-latest`) when the catalog is unreachable. Use any model ID Mistral exposes; the `-latest` aliases always track the current release.
