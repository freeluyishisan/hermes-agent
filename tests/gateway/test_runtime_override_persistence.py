...
def _persisted_model_override(model="gpt-5.5"):
    override = _model_override(model)
...
def test_sessiondb_persists_gateway_model_and_reasoning_overrides(tmp_path):
    ...
    try:
        assert reopened.get_gateway_session_model_overrides()[session_key] == _persisted_model_override()
...
def test_gateway_loads_persisted_runtime_overrides_on_startup():
    ...
    assert runner._session_model_overrides[session_key] == _persisted_model_override()
...
async def test_text_model_switch_writes_session_override_through_to_db(monkeypatch):
    ...
    session_key = build_session_key(_make_source())
    assert db.set_model_calls == [(session_key, _persisted_model_override())]
