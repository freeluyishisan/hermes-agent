...
class TestAdapterInit:
    ...
    def test_create_agent_threads_reasoning_callback(self, monkeypatch):
        """``_create_agent`` must forward ``reasoning_callback`` to AIAgent.

        Without this the api_server gateway builds the agent with no
        ``reasoning_callback``, so ``AIAgent._fire_reasoning_delta`` fires
        into ``None`` and reasoning-summary deltas (requested via
        ``summary: "auto"`` on the codex Responses backend) are silently
        discarded on the /v1/responses path.
        """
        captured = {}
        ...
        monkeypatch.setattr(
            "gateway.run.GatewayRunner._load_reasoning_config",
            staticmethod(lambda: {"enabled": True, "effort": "high", "summary": "auto"}),
        )
        monkeypatch.setattr("gateway.run.GatewayRunner._load_fallback_model", staticmethod(lambda: None))
        monkeypatch.setattr("hermes_cli.tools_config._get_platform_tools", lambda *_: set())
        ...
        sentinel = lambda _text: None  # noqa: E731
...
class TestResponsesStreaming:
    ...
    @pytest.mark.asyncio
    async def test_stream_emits_reasoning_summary_delta(self, adapter):
        """A reasoning delta fired via ``reasoning_callback`` must surface on
        the /v1/responses SSE stream as a spec-named
        ``response.reasoning_summary_text.delta`` event.

        Regression guard: the api_server gateway previously built the agent
        without a ``reasoning_callback``, so reasoning-summary deltas were
        generated by the backend and then discarded (zero reasoning frames
        on the wire).  This exercises the full wiring —
        ``_handle_responses`` closure -> queue marker -> ``_dispatch``
        SSE event — with a mocked agent (no live backend).
        """
        import json as _json
        ...
        async with TestClient(TestServer(app)) as cli:
            async def _mock_run_agent(**kwargs):
                ...
                text_cb = kwargs.get("stream_delta_callback")
                # The agent fires reasoning deltas before/around the answer.
                if reasoning_cb:
                    reasoning_cb("Let me think")
                    reasoning_cb(" about this.")
                if text_cb:
                    text_cb("Answer.")
                return (
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "hermes-agent", "input": "think hard", "stream": True},
                )
                assert resp.status == 200
                body = await resp.text()
            ...
            # Collect the reasoning-delta payloads and verify the spec shape.
            deltas = []
            lines = body.splitlines()
            for i, line in enumerate(lines):
                if line.strip() != "event: response.reasoning_summary_text.delta":
                    continue
                for follow in lines[i + 1: i + 3]:
                    if follow.startswith("data: "):
                        payload = _json.loads(follow[len("data: "):])
                        assert payload["type"] == "response.reasoning_summary_text.delta"
                        assert payload["item_id"]
                        assert payload["summary_index"] == 0
                        deltas.append(payload["delta"])
                        break

            assert deltas == ["Let me think", " about this."], deltas
            # Reasoning must not crowd out the actual answer text.
            assert "event: response.output_text.delta" in body
            assert "Answer." in body

    @pytest.mark.asyncio
    async def test_stream_without_reasoning_emits_no_reasoning_event(self, adapter):
        """Backward-safety: when the agent fires no reasoning deltas, the
        stream contains zero reasoning events (no behavior change)."""
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            async def _mock_run_agent(**kwargs):
                text_cb = kwargs.get("stream_delta_callback")
                if text_cb:
                    text_cb("Plain answer.")
                return (
                    {"final_response": "Plain answer.", "messages": [], "api_calls": 1},
                    {"input_tokens": 1, "output_tokens": 1, "total_tokens": 2},
                )
            ...
            assert "response.reasoning_summary_text.delta" not in body
