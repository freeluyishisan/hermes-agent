...
class TestComposeChannelPromptSelfIdentity:
    ...
    def test_group_with_bot_open_id_includes_self_identity(self):
        ...
        result = _compose_channel_prompt_for_chat(
            {}, "oc_g", bot_open_id="ou_self", is_group=True
        )
        assert 'Your own Feishu open_id is "ou_self"' in result
        assert '<at user_id="ou_self">' in result
    ...
    def test_self_identity_after_configured_prompt(self):
        ...
        # Order: configured persona → self-identity → @ tutorial.
    ...
    def test_dm_omits_self_identity(self):
        ...
        # p2p / non-group: no disambiguation needed, so no self-identity noise.
        result = _compose_channel_prompt_for_chat(
            {}, "oc_dm", bot_open_id="ou_self", is_group=False
        )
        assert result == _FEISHU_AT_TUTORIAL
    ...
    def test_group_without_bot_open_id_omits_self_identity(self):
        ...
        # open_id not yet hydrated → can't claim an identity, so skip it.
        result = _compose_channel_prompt_for_chat(
            {}, "oc_g", bot_open_id="", is_group=True
        )
        assert result == _FEISHU_AT_TUTORIAL
