...
async def test_session_hygiene_timeout_continues_to_agent_and_sets_cooldown(monkeypatch, tmp_path):
    """A slow auxiliary compression call must not make the gateway appear stuck.
    Hygiene should time out, warn, set a cooldown, and continue to the agent."""
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)
    ...
    # The timed-out worker may still be finishing in the thread pool, so the
    # gateway must not close its resources from underneath it.
