...
# 1.0.1, and the follow-on HIGH/MODERATE CVEs (48818 StaticFiles UNC SSRF,
# 54283 form() urlencoded DoS, 48817 method dispatch, 54282 URL hostname
# poisoning) were all closed by 1.3.1. Anything below 1.3.1 re-exposes at
# least one of these. Starlette is a transitive dep (fastapi in [web];
# sse-starlette/mcp in [mcp]/[computer-use]/[dev]) so we pin it directly in
...
def test_starlette_pinned_above_cve_2026_48710_floor_in_pyproject():
    ...
    Regression guard for #35067 / CVE-2026-48710 and the follow-on HIGH CVEs
    (48818/54283/48817/54282). A future edit that drops the pin (re-exposing
    the unbounded transitive ``starlette>=0.27`` from mcp / ``>=0.40.0`` from
    fastapi) or pins a pre-1.3.1 version fails here instead of shipping a
    Host-header auth-bypass / DoS to dashboard / MCP-HTTP users.
    """
    data = tomllib.loads((REPO_ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    ...
    for extra in ("web", "mcp", "computer-use", "dev"):
        assert extra in found, (
            f"[{extra}] no longer pins starlette directly — CVE-2026-48710/48818/54283 "
    ...
    for extra, ver in found.items():
        assert _version_tuple(ver) >= _STARLETTE_CVE_FLOOR, (
            f"[{extra}] pins starlette=={ver}, below the Starlette CVE fix "
...
def test_locked_starlette_is_not_vulnerable_to_cve_2026_48710():
    ...
    resolved version is >= the Starlette CVE fix floor so a stale-lock
    ...
    for ver in versions:
        assert _version_tuple(ver) >= _STARLETTE_CVE_FLOOR, (
            f"uv.lock resolves starlette=={ver}, below the Starlette CVE fix "
