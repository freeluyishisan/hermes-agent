...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        def slow_run(**kwargs):
            # Wait until the heartbeat has definitely crossed the idle stale
            # ceiling.  This avoids relying on a fixed wall-clock sleep on
            # busy CI runners: the test only completes early after observing
            # enough real heartbeats.
            deadline = time.monotonic() + 1.0
            while len(touch_calls) <= idle_limit + 1 and time.monotonic() < deadline:
                time.sleep(0.01)
            return {"final_response": "done", "completed": True, "api_calls": 1}

        child.run_conversation.side_effect = slow_run

        # Keep the idle ceiling deliberately tiny.  If the implementation used
        # the idle stale limit while current_tool is set, the heartbeat would
        # stop after at most ``idle_limit`` touches.  The in-tool threshold must
        # allow additional touches while the child is legitimately inside a
        # long-running tool.
        with (
            patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.01),
            patch("tools.delegate_tool._HEARTBEAT_STALE_CYCLES_IDLE", idle_limit),
        ...
        self.assertGreater(
            ...
            f"got {len(touch_calls)} touches with idle limit {idle_limit}",
        )
