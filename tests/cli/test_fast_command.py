...
class TestPriorityProcessingModels(unittest.TestCase):
    ...
    def test_all_anthropic_models_supported(self):
        """The speed=fast parameter is gated to Opus 4.6 and Opus 4.8.

        Sending speed=fast to Opus 4.7, Sonnet, or Haiku returns HTTP 400.
        The separate ``…-opus-4.8-fast`` model id already bakes in fast
        inference via the model field, so the speed param is excluded there.
        """
        from hermes_cli.models import model_supports_fast_mode

        # Supported: Opus 4.6 and Opus 4.8 (base ids), in any vendor form
        supported = [
        ...
        # Unsupported per Anthropic API: Opus 4.7, the -fast alias (fast via
        # model field, not the param), Sonnet, Haiku
        unsupported = [
            "claude-opus-4-7",
...
class TestAnthropicFastMode(unittest.TestCase):
    ...
    def test_anthropic_opus_supported(self):
        ...
        assert model_supports_fast_mode("anthropic/claude-opus-4.6") is True
        # Opus 4.8 (research preview) also accepts the speed=fast parameter
    ...
    def test_anthropic_non_fast_param_models_excluded(self):
        """The speed=fast parameter is gated to Opus 4.6 and 4.8 — others out.

        Per https://platform.claude.com/docs/en/build-with-claude/fast-mode,
        sending speed=fast to Opus 4.7, Sonnet, or Haiku returns HTTP 400. The
    ...
    def test_resolve_overrides_returns_none_for_unsupported_claude(self):
        """Opus 4.7, the -fast alias, and other Claude models don't take speed.

        The speed=fast parameter is gated to Opus 4.6 and Opus 4.8; the separate
        ``…-opus-4.8-fast`` model id serves fast inference via the model field,
        so the param must not be added on top.
        """
        from hermes_cli.models import resolve_fast_mode_overrides
    ...
    def test_is_anthropic_fast_model(self):
        """The speed=fast parameter is gated to Opus 4.6 and 4.8 base ids."""
        from hermes_cli.models import _is_anthropic_fast_model

        # Supported: Opus 4.6 / 4.8 base ids in any form
        ...
        # Unsupported — would 400 (4.7), or the -fast alias (fast via model id)
