...
Regression for #34536: older POSIX installers dropped node/npm/npx symlinks
in ~/.local/bin pointing into $HERMES_HOME/node and prepended ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those legacy symlinks,
but only when they still resolve into the Hermes-managed node dir.
"""
