...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_whoami_command(self, event: MessageEvent) -> str:
        ...
        if not policy.enabled:
            return t("gateway.whoami_unrestricted", platform=platform, scope=scope, user_id=user_id)

        if policy.is_admin(user_id):
            return t("gateway.whoami_admin", platform=platform, scope=scope, user_id=user_id)
        ...
        return t("gateway.whoami_user", platform=platform, scope=scope, user_id=user_id, runnable_str=runnable_str)
            ...
            if failed:
                for p, info in failed.items():
                    if info.get("paused"):
                        ...
                        lines.append(t("gateway.platform_failed_paused", platform=p.value, reason=reason))
                    else:
                        attempts = info.get("attempts", 0)
                        lines.append(t("gateway.platform_retrying", platform=p.value, attempts=attempts))
            else:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if _cost_warning is not None:
            ...
            return await self._request_slash_confirm(
                ...
                message=t("gateway.model_expensive_warning",
                          warning_message=_cost_warning.message, prefix=_p),
    ...
    async def _handle_subgoal_command(self, event: "MessageEvent") -> str:
        ...
        if verb == "clear":
            ...
            if prev:
                key = "gateway.subgoal_cleared_one" if prev == 1 else "gateway.subgoal_cleared_many"
                return t(key, count=prev)
