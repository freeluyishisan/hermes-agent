...
class GatewaySlashCommandsMixin:
    ...
    def _resume_row_visible(
        ...
        if allow_all and self._resume_caller_is_admin(source):
            return True
        return self._resume_target_allowed(source, sid, allow_override=False)
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        if not name:
            ...
            try:
                ...
                titled = [
                    s for s in titled if self._resume_row_visible(source, s, allow_all)
                ]
                if not titled:
        ...
        elif not self._resume_target_allowed(
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        if not cross_origin:
            ...
            rows = [
                row for row in rows
                if self._resume_row_visible(source, row, allow_all=False)
            ]
