...
def _is_local_backend() -> bool:
    ...
    # the BROWSER_CDP_URL env var, but _get_cdp_override() also honors a
    # persistent `browser.cdp_url` config — so CAMOFOX_URL + a config CDP
    # override would otherwise return local and skip the private/internal SSRF
    # gate. _is_local_mode() already treats any CDP override as non-local; keep
    # the two helpers in agreement.
    if _get_cdp_override():
        return False
    if _is_camofox_mode():
        return True
