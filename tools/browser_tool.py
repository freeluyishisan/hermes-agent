...
def _resolve_cdp_override(cdp_url: str) -> str:
    ...
    try:
        ...
        logger.debug("CDP endpoint %s is not reachable via %s: %s", raw, version_url, exc)
        with _cdp_discovery_failure_cache_lock:
    ...
    logger.debug("CDP discovery at %s did not return webSocketDebuggerUrl", version_url)
    with _cdp_discovery_failure_cache_lock:
        _cdp_discovery_failure_cache[raw] = time.monotonic() + _CDP_DISCOVERY_FAILURE_CACHE_TTL_S
    return ""
