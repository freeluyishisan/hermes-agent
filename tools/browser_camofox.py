...
def _camofox_headers() -> Dict[str, str]:
    """Return auth headers for Camofox API.

    Reads the API key from the CAMOFOX_API_KEY environment variable.
    """
...
def _post(path: str, body: dict, timeout: int = _DEFAULT_TIMEOUT) -> dict:
    ...
    resp = requests.post(url, json=body, timeout=timeout, headers=_camofox_headers())
    resp.raise_for_status()
    return resp.json()
...
def _get(path: str, params: dict = None, timeout: int = _DEFAULT_TIMEOUT) -> dict:
    ...
    url = f"{get_camofox_url()}{path}"
    resp = requests.get(url, params=params, timeout=timeout, headers=_camofox_headers())
    resp.raise_for_status()
    return resp.json()
...
def _delete(path: str, body: dict = None, timeout: int = _DEFAULT_TIMEOUT) -> dict:
    ...
    url = f"{get_camofox_url()}{path}"
    resp = requests.delete(url, json=body, timeout=timeout, headers=_camofox_headers())
    resp.raise_for_status()
    return resp.json()
