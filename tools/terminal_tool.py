    ...
    try:
        ...
        if env_type == "local":
            ...
            if not docker:
                ...
                return False
            result = subprocess.run([docker, "version"], capture_output=True, timeout=5, creationflags=_CREATE_NO_WINDOW)
            return result.returncode == 0

        elif env_type == "singularity":
            executable = shutil.which("apptainer") or shutil.which("singularity")
            if executable:
                result = subprocess.run([executable, "--version"], capture_output=True, timeout=5, creationflags=_CREATE_NO_WINDOW)
                return result.returncode == 0
            return False
