...
export const ru: Translations = {
  ...
  settings: {
    ...
    fieldLabels: {
      ...
      'display.personality': 'Стиль личности',
      'display.showReasoning': 'Блоки рассуждений',
      'agent.maxTurns': 'Макс. шагов агента',
      'agent.imageInputMode': 'Прикрепление изображений',
      'agent.apiMaxRetries': 'Повторные попытки API',
      'agent.serviceTier': 'Уровень сервиса',
      'agent.toolUseEnforcement': 'Применение инструментов',
      ...
      'terminal.dockerImage': 'Docker-образ',
      'terminal.singularityImage': 'Singularity-образ',
      'terminal.modalImage': 'Modal-образ',
      'terminal.daytonaImage': 'Daytona-образ',
      ...
      'approvals.mode': 'Режим подтверждения',
      ...
      'security.allowPrivateUrls': 'Разрешать частные URL',
      'browser.allowPrivateUrls': 'Частные URL в браузере',
      'browser.autoLocalForPrivateUrls': 'Локальный браузер для частных URL',
      ...
      'stt.elevenlabs.tagAudioEvents': 'Разметка аудиособытий',
      'stt.elevenlabs.diarize': 'Диаризация докладчиков',
      ...
      'memory.userCharLimit': 'Бюджет профиля',
      ...
      'compression.protectLastN': 'Защита последних сообщений',
      'delegation.model': 'Модель подагента',
      'delegation.provider': 'Провайдер подагента',
      'delegation.maxIterations': 'Лимит шагов подагента',
      'delegation.maxConcurrentChildren': 'Параллельные подагенты',
      'delegation.childTimeoutSeconds': 'Тайм-аут подагента',
      'delegation.reasoningEffort': 'Усилие рассуждений подагента',
      'updates.nonInteractiveLocalChanges': 'Локальные изменения при обновлении',
    },
    fieldDescriptions: {
      ...
      'fallbackProviders': 'Резервные записи провайдер:модель для использования при сбое основной модели.',
      'display.personality': 'Стиль ассистента по умолчанию для новых сессий.',
      'display.showReasoning': 'Показывать блоки рассуждений, если сервер их предоставляет.',
      'timezone': 'Используется, когда Hermes нужен местный временной контекст. Оставьте пустым для системного часового пояса.',
      'agent.imageInputMode': 'Управляет передачей прикреплённых изображений в модель.',
      'agent.maxTurns': 'Верхний предел шагов с инструментами до остановки Hermes.',
      'terminal.cwd': 'Папка проекта по умолчанию для работы с инструментами и терминалом.',
      'terminal.persistentShell': 'Сохранять состояние оболочки между командами, если бэкенд поддерживает.',
      'terminal.envPassthrough': 'Переменные окружения для передачи в выполнение инструментов.',
      'terminal.dockerImage': 'Образ контейнера для бэкенда Docker.',
      'terminal.singularityImage': 'Образ для бэкенда Singularity.',
      'terminal.modalImage': 'Образ для бэкенда Modal.',
      'terminal.daytonaImage': 'Образ для бэкенда Daytona.',
      'codeExecution.mode': 'Насколько строго выполнение кода ограничено текущим проектом.',
      'fileReadMaxChars': 'Максимальное число символов Hermes может прочитать из одного запроса файла.',
      'approvals.mode': 'Как Hermes обрабатывает команды, требующие явного подтверждения.',
      'approvals.timeout': 'Сколько ждёт запрос подтверждения перед таймаутом.',
      'security.redactSecrets': 'Скрывать обнаруженные секреты из видимого модели контента, если возможно.',
      'checkpoints.enabled': 'Создавать снимки для отката перед редактированием файлов.',
      'memory.memoryEnabled': 'Сохранять долгосрочные воспоминания для будущих сессий.',
      'memory.userProfileEnabled': 'Поддерживать компактный профиль предпочтений пользователя.',
      'context.engine': 'Стратегия управления длинными разговорами при приближении к лимиту контекста.',
      'compression.enabled': 'Суммировать старый контекст при увеличении разговора.',
      'voice.autoTts': 'Автоматически озвучивать ответы ассистента.',
      'tts.xai.voiceId': 'ID голоса xAI (например, eve) или собственный ID голоса.',
      'tts.xai.language': 'Код разговорного языка, например ru.',
      'tts.neutts.device': 'Локальное устройство вывода для NeuTTS.',
      'stt.enabled': 'Включить локальную или провайдерскую транскрипцию речи.',
      'stt.elevenlabs.languageCode': 'Необязательный ISO-639-3 код языка. Пустое поле — автоопределение ElevenLabs.',
      'updates.nonInteractiveLocalChanges': 'При автоматическом обновлении изнутри приложения — сохранить локальные изменения (stash) или удалить (discard). Обновление через терминал всегда спрашивает.',
    },
    about: {
