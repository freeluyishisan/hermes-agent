...
it is **gated on git workspace detection** — if the agent's cwd is inside
a git repository, LSP runs against that workspace; otherwise the
file_operations layer falls back to its existing in-process syntax
checks. This keeps users whose cwd is their home directory (e.g. Telegram
gateway chats) from spawning daemons they don't need.
