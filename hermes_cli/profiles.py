...
def _get_path_guidance() -> str:
    ...
    if sys.platform == "win32":
        ...
        return (
            f"  Add to your environment (PowerShell, run as Administrator):\n"
            f'    [Environment]::SetEnvironmentVariable("Path",\n'
            f'      [Environment]::GetEnvironmentVariable("Path","Process") + ";{wrapper_str}")\n'
            f"\n"
            f"  Or add to ~/.bashrc for Git Bash:\n"
            f'    export PATH="{wrapper_str}:$PATH"\n'
        )
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            ...
            bat_content = (
                f"@echo off\r\n"
                f":: Hermes profile wrapper for '{profile}' - auto-generated, do not edit\r\n"
                f"setlocal\r\n"
