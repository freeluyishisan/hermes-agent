...
def _needs_launchd_wrapper(python_path: str) -> bool:
    """Return True when launchd should exec a boot-volume wrapper script.

    launchd can fail with EX_CONFIG (78) when ProgramArguments points at a
    Python binary or venv symlink under /Volumes.  Check both the lexical path
    and the resolved target so symlinked venv interpreters are covered.
    """
    if not is_macos():
        return False
    try:
        ...
        return False
    return (
        str(python_path).startswith("/Volumes/")
        or str(resolved).startswith("/Volumes/")
    )


def _launchd_wrapper_path() -> Path:
    """Return the profile-scoped launchd wrapper path on the boot volume."""
    import re
    ...
    return (
        ...
        / ".local"
        / "bin"
        / f"hermes-gateway-launchd-{safe_label}.sh"
    )


def _build_launchd_sane_path() -> str:
...
def _parse_plugin_env_line(raw_line: str) -> tuple[str, str] | None:
    """Parse a simple KEY=VALUE plugin .env line without executing shell."""
    import re
...
def _plugin_env_exports_for_launchd_wrapper(hermes_home: Path) -> list[str]:
    """Return shell-quoted export lines for plugin .env files."""
    exports: list[str] = []
    ...
    for plugin_env in sorted(plugins_dir.glob("*/.env")):
        exports.append(f"# Plugin env: {plugin_env.parent.name}")
        try:
            raw_lines = plugin_env.read_text(encoding="utf-8").splitlines()
        except OSError:
            exports.append(f"# Could not read {shlex.quote(str(plugin_env))}")
            continue
        for raw_line in raw_lines:
            parsed = _parse_plugin_env_line(raw_line)
            if parsed is None:
                continue
            key, value = parsed
...
def _external_volume_mount_point(path: str) -> str | None:
    try:
...
def _create_launchd_wrapper(python_path: str, profile_arg: str) -> str:
    """Create the profile-scoped shell wrapper used by launchd on /Volumes."""
    ...
    plugin_exports = _plugin_env_exports_for_launchd_wrapper(hermes_home)
    plugin_block = "\n".join(plugin_exports) if plugin_exports else "# No plugin .env files found"

    command = [
        shlex.quote(python_path),
        "-m",
        "hermes_cli.main",
        *(shlex.quote(part) for part in profile_arg.split()),
        "gateway",
        "run",
        "--replace",
    ]
    ...
    wait_lines: list[str] = []
    mount_point = _external_volume_mount_point(python_path)
    if mount_point:
        wait_lines = [
            f"_mount_point={shlex.quote(mount_point)}",
            f"_python_path={shlex.quote(python_path)}",
            'while [ ! -x "$_python_path" ]; do',
            '  echo "Waiting for $_mount_point before starting Hermes gateway..." >&2',
            "  sleep 5",
            "done",
            "",
        ]
    ...
    lines = [
        ...
        *wait_lines,
        ...
        f"export PATH={shlex.quote(_build_launchd_sane_path())}",
        "",
...
def generate_launchd_plist() -> str:
    ...
    detected_venv = _detect_venv_dir()
    venv_dir = str(detected_venv) if detected_venv else str(PROJECT_ROOT / "venv")
    sane_path = _build_launchd_sane_path()

    # Build ProgramArguments array, including --profile when using a named profile
    if _needs_launchd_wrapper(python_path):
        wrapper_path = _create_launchd_wrapper(python_path, profile_arg)
        prog_args = [
            "<string>/bin/zsh</string>",
            f"<string>{wrapper_path}</string>",
        ]
        # Keep launchd's own cwd/log paths on the boot volume. The wrapper
        # waits for the external Python path and then exports the real runtime
        # environment before exec-ing Hermes.
